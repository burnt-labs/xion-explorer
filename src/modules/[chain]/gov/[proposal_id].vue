  ...
  <div>
    <div class="bg-base-100 px-4 pt-3 pb-4 rounded mb-4 shadow">
      ...
      <div v-if="proposal.summary && !proposal.content?.description || metaItem(proposal?.metadata)?.summary">
        <MdEditor :model-value="format.multiLine(proposal.summary || metaItem(proposal?.metadata)?.summary)" previewOnly
          class="md-editor-recover"></MdEditor>
      </div>
    </div>
        ...
        <table class="table w-full table-zebra">
          <tbody>
            <tr v-for="(item, index) of votes" :key="index">
              ...
              <td v-if="item.option && item.option !== 'VOTE_OPTION_UNSPECIFIED'" class="py-2 text-sm" :class="{
