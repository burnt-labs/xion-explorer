...
<template>
  <div>
    <div v-if="coinInfo && coinInfo.name" class="bg-base-100 rounded shadow">
      <div class="grid grid-cols-2 md:grid-cols-3 p-4">
        <div class="col-span-2 md:col-span-1">
          ...
          <div class="text-xs mt-2">
            {{ $t('index.rank') }}:
            <div class="badge text-xs badge-error bg-error/10 text-error">
          ...
          <div class="my-4 flex flex-wrap items-center">
            <a
              ...
              :key="index"
              :href="item.href"
              class="link link-primary px-2 py-1 rounded-sm no-underline hover:text-primary hover:bg-active dark:hover:bg-slate-800 flex items-center"
            >
              <Icon :icon="item?.icon" />
          ...
          <div>
            <div class="dropdown dropdown-hover w-full">
              <label>
                <div
                  class="bg-base-200 dark:bg-base-300 flex items-center justify-between px-4 py-2 cursor-pointer rounded"
                >
                  <div>
                    <div class="font-semibold text-xl text-base-content">
                      {{ ticker?.market?.name || '' }}
                    </div>
      ...
      <!--
      <div class="h-[1px] w-full bg-base-200 dark:bg-base-300"></div>
      ...
      <div class="mx-4 flex flex-wrap items-center">
        <div v-for="tag in coinInfo.categories"
          class="mr-2 mb-4 text-xs bg-base-200 dark:bg-base-300 px-3 rounded-full py-1">
          {{ tag }}
        </div>
      </div>
      -->
    ...
    <div v-if="store.proposals?.proposals?.length > 0" class="bg-base-100 rounded mt-4 shadow">
    ...
    <div class="bg-base-100 rounded mt-4 shadow">
      ...
      <div class="grid grid-cols-1 md:!grid-cols-4 auto-cols-auto gap-4 px-4 pb-6">
        <div class="bg-base-200 dark:bg-base-300 rounded-sm px-4 py-3">
                ...
                {{
                  format.formatTokens(
                    walletStore?.rewards?.rewards?.find(
                      (el: any) => el?.validator_address === item?.delegation?.validator_address
                    )?.reward
                  )
                }}
              </td>
