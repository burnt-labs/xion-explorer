...
import { coingeckoUrl } from '@/stores/xion/useCoinGecko';
...
export const useDashboard = defineStore('dashboard', {
  state: () => {
    const favMap = JSON.parse(
      ...
      '{"xion-mainnet-1":true, "xion-testnet-2":true}'
    );
    return {
  ...
  actions: {
    ...
    loadingPrices() {
      ...
      get(`${coingeckoUrl}/api/v3/simple/price?include_24hr_change=true&vs_currencies=${currencies.join(',')}&ids=${coinIds.join(",")}`).then(x => {
        this.prices = x
      })
    },
    ...
    async loadLocalConfig(network: NetworkType) {
      ...
      const source: Record<string, LocalConfig> = import.meta.glob('../../../chains/xion/*.json', { eager: true });
      Object.values<LocalConfig>(source).forEach((x: LocalConfig) => {
